...
bool Octree::contains(const vec3 &center, const float &le, const vec3 &ve)
{
	if (ve.e[0] >= center.e[0] + le / 2 || ve.e[0] < center.e[0] - le / 2)
		return false;
	if (ve.e[1] >= center.e[1] + le / 2 || ve.e[1] < center.e[1] - le / 2)
		return false;
	if (ve.e[2] >= center.e[2] + le / 2 || ve.e[2] < center.e[2] - le / 2)
		return false;
...
void Octree::divide()
{
	if (triangles.size() > 4 && depth < 4)
	{
		...
		for (int i = 0; i < 8; i++)
		{
			...
			{
				if (contains(center + vec3(
					(length / 4)*neg[i * 3],
					(length / 4)*neg[i * 3 + 1],
					(length / 4)*neg[i * 3 + 2]),
					length / 2, triangles.at(j)->center))
